/**
 * ============================================================================
 * @file    dlm_wifi_prototype.c
 * @brief   WIFI 数据链路管理器 (DLM) - 原型阶段实现
 *
 * @details 遵循 ARINC 839-2014 MIH 协议规范的 WIFI 链路原型实现
 *          - 配置管理器 (CM): 管理静态配置参数
 *          - 状态模拟器 (SS): 管理动态运行状态
 *          - 原型阶段"仅报告不操作"原则
...
typedef enum {
  FLIGHT_PHASE_GROUND = 0,   /* 地面阶段 - WIFI 可用 */
  FLIGHT_PHASE_TAXI = 1,     /* 滑行阶段 - WIFI 可用但信号弱 */
  FLIGHT_PHASE_TAKEOFF = 2,  /* 起飞阶段 - WIFI 不可用 */
  FLIGHT_PHASE_CLIMB = 3,    /* 爬升阶段 - WIFI 不可用 */
  FLIGHT_PHASE_CRUISE = 4,   /* 巡航阶段 - WIFI 不可用 */
  FLIGHT_PHASE_DESCENT = 5,  /* 下降阶段 - WIFI 不可用 */
  FLIGHT_PHASE_APPROACH = 6, /* 进场阶段 - WIFI 可能可用 */
  FLIGHT_PHASE_LANDING = 7   /* 着陆阶段 - WIFI 可用 */
} flight_phase_t;

static flight_phase_t g_flight_phase = FLIGHT_PHASE_GROUND;
...
static const char *flight_phase_to_string(flight_phase_t phase);
...
static bool is_wifi_available(void) {
  /* WIFI 仅在地面阶段可用 */
  return (g_flight_phase == FLIGHT_PHASE_GROUND ||
          g_flight_phase == FLIGHT_PHASE_TAXI ||
          g_flight_phase == FLIGHT_PHASE_LANDING);
}
...
static int dlm_init_config_manager(const char *config_path) {
  ...
  printf("[WIFI-CM] 配置管理器初始化完成:\n");
  printf("  - Link ID: 0x%02X, 接口: %s\n", g_ctx.config.link_id,
         g_ctx.config.interface_name);
  printf("  - IP: %s, 网关: %s\n", g_net_config.ip_address,
         g_net_config.gateway);
  printf("  - 带宽 FL/RL: %u/%u kbps\n", g_ctx.config.max_bandwidth_fl,
         g_ctx.config.max_bandwidth_rl);
  printf("  - 延迟: %u ms, RSSI阈值: %d dBm\n", g_ctx.config.reported_delay_ms,
         g_ctx.config.rssi_threshold_dbm);
  printf("  - 仅地面可用: %s\n", g_ctx.config.ground_only ? "是" : "否");
...
static int dlm_init_state_simulator(void) {
  ...
  g_ctx.state.signal_quality = 90; /* WIFI 通常信号很好 */

  printf("[WIFI-SS] 状态模拟器初始化完成:\n");
  printf("  - RSSI: %d dBm, 信号质量: %u%%\n", g_ctx.state.simulated_rssi,
         g_ctx.state.signal_quality);
  printf("  - 飞行阶段: %s, WIFI可用: %s\n",
         flight_phase_to_string(g_flight_phase),
         is_wifi_available() ? "YES" : "NO");
...
static int dlm_physical_link_up(void) {
  ...
  if (!is_wifi_available()) {
    printf("[WIFI-PHY] 警告: 当前飞行阶段(%s)不允许使用WIFI\n",
           flight_phase_to_string(g_flight_phase));
    return -1;
  }
  ...
  snprintf(ind.link_identifier.poa_addr, sizeof(ind.link_identifier.poa_addr),
           "%s",
           g_ctx.config.interface_name); /* 在 poa_addr 中存储网络接口名 */
...
static int dlm_physical_link_down(int reason_code) {
  ...
  snprintf(cmd, sizeof(cmd), "ip addr del %s/%s dev %s 2>/dev/null",
           ...
           g_ctx.config.interface_name);
  system(cmd);
  printf("[WIFI-PHY] 删除IP: %s\n", g_net_config.ip_address);

  /*
   * 注意: 不再关闭物理接口
   * 原因: 接口可能被其他服务使用，DLM 退出不应影响物理层
   */
  printf("[WIFI-PHY] 保持接口 %s 状态不变\n", g_ctx.config.interface_name);
...
static int dlm_handle_capability_discover(void) {
  ...
  snprintf(confirm.link_identifier.link_addr,
           sizeof(confirm.link_identifier.link_addr), "%s",
           g_ctx.config.link_name); /* 使用 link_name 作为标识符 */
  snprintf(confirm.link_identifier.poa_addr,
           sizeof(confirm.link_identifier.poa_addr), "%s",
           g_ctx.config.interface_name); /* 在 poa_addr 中存储网络接口名 */
...
static int dlm_handle_link_resource(const LINK_Resource_Request *req) {
  ...
  if (req->resource_action == RESOURCE_ACTION_REQUEST) {
    ...
    uint8_t bearer_id = 0;
    uint32_t req_bw_fl =
        req->has_qos_params ? req->qos_parameters.forward_link_rate : 10000;
    uint32_t req_bw_rl =
        req->has_qos_params ? req->qos_parameters.return_link_rate : 5000;
    uint8_t cos_id =
    ...
    if (ret == 0) {
      ...
      printf("  - [原型模式] 分配 Bearer ID: %u (WIFI 高速链路)\n", bearer_id);
    } else {
...
static int dlm_send_going_down_indication(uint32_t time_to_down_ms,
                                          int reason) {
  printf("[WIFI-IND] 发送 Link_Going_Down.indication (剩余=%ums)\n",
         time_to_down_ms);
  ...
  ind.confidence = 90; /* WIFI 信号变化快,预测准确 */

  if (reason == LINK_DOWN_REASON_COVERAGE_LOST) {
    snprintf(ind.reason_text, sizeof(ind.reason_text),
             "Flight phase changing - WIFI coverage lost");
  } else {
    snprintf(ind.reason_text, sizeof(ind.reason_text),
...
static int dlm_send_detected_indication(void) {
  printf("[WIFI-IND] 发送 Link_Detected.indication\n");

  LINK_Detected_Indication ind;
  memset(&ind, 0, sizeof(ind));
  ind.link_identifier.link_type = g_ctx.config.link_id;
  snprintf(ind.link_identifier.link_addr, sizeof(ind.link_identifier.link_addr),
           "%s", g_ctx.config.link_name); /* 使用 link_name 作为标识符 */
  snprintf(ind.link_identifier.poa_addr, sizeof(ind.link_identifier.poa_addr),
           "%s",
           g_ctx.config.interface_name); /* 在 poa_addr 中存储网络接口名 */
  ind.link_type = LINK_PARAM_TYPE_802_11;
  ind.max_bandwidth_kbps = g_ctx.config.max_bandwidth_fl;
  ind.signal_strength_dbm = g_ctx.state.simulated_rssi;
  ind.signal_quality = g_ctx.state.signal_quality;
  ind.security_supported = g_ctx.config.security_level;
  ind.detection_timestamp = (uint32_t)time(NULL);

  return dlm_send_to_mihf(MIH_LINK_DETECTED_IND, &ind, sizeof(ind));
}
...
static int dlm_send_parameters_report(void) {
  ...
  snprintf(ind.link_identifier.poa_addr, sizeof(ind.link_identifier.poa_addr),
           ...
           g_ctx.config.interface_name); /* 在 poa_addr 中存储网络接口名 */
  ind.changed_params =
      LINK_PARAM_QUERY_SIGNAL_STRENGTH | LINK_PARAM_QUERY_AVAILABLE_BW;
  ind.parameters.signal_strength_dbm = g_ctx.state.simulated_rssi;
  ind.parameters.signal_quality = g_ctx.state.signal_quality;
...
static void dlm_append_hex_words_from_line(const char *line, uint8_t *out,
...
static size_t dlm_guess_ipv4_offset(const uint8_t *pkt, size_t len) {
  if (len >= 18 && pkt[12] == 0x81 && pkt[13] == 0x00 && pkt[16] == 0x08 &&
      pkt[17] == 0x00) {
    return 18;
  }
...
static void dlm_print_payload_summary(const char *tag, const uint8_t *payload,
                                      size_t payload_len) {
  ...
  if (payload_len > hex_len)
    printf(" ...");
  printf("\n");
}

static void dlm_try_parse_udp_payload_from_tcpdump(const char *tag,
                                                   char **lines,
                                                   int line_count) {
  ...
  for (int i = 0; i < line_count; i++) {
    ...
    if (!hdr)
      continue;
    if (strstr(hdr, " IP ") == NULL)
      continue;
    if (strstr(hdr, " UDP") == NULL)
      continue;
    if (strstr(hdr, "length") == NULL)
      continue;
    ...
    if (payload_off + payload_len > pkt_len) {
      if (pkt_len > payload_off)
        payload_len = pkt_len - payload_off;
      else
        payload_len = 0;
    }
...
static void *dlm_packet_monitor_thread(void *arg) {
  ...
  printf("[WIFI-PKT] 数据包监控线程启动，监控接口: %s\n",
         g_ctx.config.interface_name);

  /*
   * 使用持续运行的 tcpdump 进程，避免反复启动导致的抓包空档
   * -l: 行缓冲，确保实时输出
   * -n: 不解析域名
   * -vv: 详细输出
   * -X: 打印 hex 和 ascii
  ...
  snprintf(cmd, sizeof(cmd),
           "tcpdump -i %s -n -s 0 -vv -X -U -l 'udp and port 5000' 2>&1",
           g_ctx.config.interface_name);

  printf("[WIFI-PKT] 启动持续抓包命令: %s\n", cmd);
  printf("[WIFI-PKT] ═══════════════════════════════════════\n");

  /* 启动一次，持续读取 */
  fp = popen(cmd, "r");
  if (fp == NULL) {
    printf("[WIFI-PKT] 错误: 无法启动 tcpdump: %s\n", strerror(errno));
    return NULL;
  }
...
#define MAX_PKT_LINES 256
  ...
  while (fgets(line, sizeof(line), fp) != NULL && g_running) {
    /*
     * tcpdump 的输出是一流式的。我们需要识别包的边界。
     * 简单起见，我们检测到新的 " IP " 行时，处理上一批累积的行，或者实时处理。
     * 为了保持现有逻辑的兼容性（dlm_try_parse_udp_payload_from_tcpdump
     * 需要多行上下文）， 我们采用滑动窗口或简单的批处理：
     * 这里使用简单的策略：每读取一行都打印，并尝试每隔几行或检测到包头时解析。
     *
     * 但为了更直观的日志，我们直接打印每一行，并维护一个小缓冲区给 payload
     * 解析器。
     */
    ...
    /* 简单的缓冲逻辑用于 payload 解析 (可选)
     * 由于流式输出，以前的 "一批处理" 逻辑需要调整。
     * 为简化，这里我们先只做实时打印，这已经能解决 "看不到包" 的问题。
     * 如果需要解析 payload，可以在检测到 "0x0000" 等行时进行累积。
     */

    /*
     * 如果行包含特定的 pattern，我们可以尝试解析上一组累积的行。
     * 现有的 dlm_try_parse_udp_payload_from_tcpdump 设计用于处理整个 buffer。
     * 修改为：累积行，直到遇到下一个 " IP " 开头，或者 buffer 满。
     */

    if (strstr(line, " IP ") != NULL && pkt_count > 0) {
      /* 遇到新包头，先处理之前的 */
      dlm_try_parse_udp_payload_from_tcpdump("WIFI", pkts, pkt_count);
      /* 清理 buffer */
    ...
    if (pkt_count < MAX_PKT_LINES) {
      ...
      /* Buffer full, force flush (drop old) */
      for (int i = 0; i < pkt_count; i++)
        free(pkts[i]);
      pkt_count = 0;
      pkts[pkt_count++] = strdup(line);
...
static int dlm_init_socket(void) {
  ...
  strncpy(g_mihf_addr.sun_path, g_net_config.mihf_path,
          sizeof(g_mihf_addr.sun_path) - 1);

  printf("[WIFI] 套接字已初始化: %s\n", g_net_config.dlm_path);
  return 0;
}
...
static void dlm_send_link_up_heartbeat(void) {
  ...
  dlm_send_to_mihf(MIH_LINK_UP_IND, &ind, sizeof(ind));
  printf("[WIFI-HB] Link_Up heartbeat sent\n");
}

static void *dlm_reporting_thread(void *arg) {
  (void)arg;
  printf("[WIFI-THR] 参数上报线程已启动\n");
  
  bool prev_iface_up = false;  /* 上一次网卡状态 */
  ...
  while (g_running) {
    ...
    if (prev_iface_up && !curr_iface_up) {
      /* 网卡从 UP 变为 DOWN */
      printf("[WIFI-MON] 检测到网卡 %s 状态变为 DOWN\n", g_ctx.config.interface_name);
      ...
      /* 网卡从 DOWN 变为 UP */
      printf("[WIFI-MON] 检测到网卡 %s 状态变为 UP\n", g_ctx.config.interface_name);
      ...
      if (was_down) {
        /* 发送 Link_Detected.indication */
        dlm_send_detected_indication();
  ...
  printf("[WIFI-THR] 参数上报线程已退出\n");
  return NULL;
}

static void *dlm_simulation_thread(void *arg) {
  (void)arg;
  printf("[WIFI-THR] 状态模拟线程已启动\n");

  static bool prev_wifi_available = true;
  ...
  while (g_running) {
    ...
    /* 检查飞行阶段变化 */
    bool current_available = is_wifi_available();

    if (prev_wifi_available && !current_available) {
      /* WIFI 将变为不可用 - 发送 Going_Down */
      if (g_ctx.state.is_connected) {
        dlm_send_going_down_indication(g_net_config.going_down_lead_time_ms,
                                       LINK_DOWN_REASON_COVERAGE_LOST);

        /* 延迟后断开 */
        usleep(g_net_config.going_down_lead_time_ms * 1000);
        dlm_physical_link_down(LINK_DOWN_REASON_COVERAGE_LOST);
      }
    } else if (!prev_wifi_available && current_available) {
      /* WIFI 变为可用 - 发送 Detected */
      dlm_send_detected_indication();
    }

    prev_wifi_available = current_available;
    ...
    if (g_ctx.state.is_connected) {
      ...
      /* 检查是否需要发送 Going_Down */
      pthread_mutex_lock(&g_ctx.state.mutex);
      bool trigger_going_down =
          (g_ctx.state.simulated_rssi < g_ctx.config.rssi_threshold_dbm) &&
...
static void dlm_print_wifi_status(void) {
...
int main(int argc, char *argv[]) {
  ...
  srand((unsigned int)time(NULL));
  {
    ...
    sa.sa_flags =
        0; /* 不设置 SA_RESTART，这样 recv() 会在信号到来时返回 EINTR */
    sigaction(SIGINT, &sa, NULL);
    sigaction(SIGTERM, &sa, NULL);
  }

  if (dlm_init_config_manager(config_path) != 0) {
    fprintf(stderr, "[WIFI] 配置管理器初始化失败\n");
    return EXIT_FAILURE;
  }

  if (dlm_init_state_simulator() != 0) {
    fprintf(stderr, "[WIFI] 状态模拟器初始化失败\n");
    return EXIT_FAILURE;
  }

  if (dlm_init_socket() != 0) {
    fprintf(stderr, "[WIFI] 套接字初始化失败\n");
    return EXIT_FAILURE;
  }
  ...
  if (is_wifi_available()) {
    printf("\n[WIFI] 原型模式: 检测到地面阶段, 自动激活链路...\n");
    dlm_physical_link_up();

    /* 链路激活后启动 UDP 监听 */
    sleep(1); /* 等待接口完全就绪 */
    if (dlm_udp_listener_start(&udp_listener, g_net_config.ip_address,
                               DLM_UDP_LISTEN_PORT, "WIFI") != 0) {
      fprintf(stderr, "[WIFI] UDP 监听启动失败（非致命错误）\n");
    }
  } else {
    printf("\n[WIFI] 警告: 当前不在地面阶段, WIFI 不可用\n");
  }

  /* 打印初始状态 */
  dlm_print_wifi_status();
