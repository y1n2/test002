/**
 * @file magic_dict_handles.h
 * @brief MAGIC 客户端 Diameter 词典句柄管理。
 * @details 定义了标准 Diameter AVP 和 MAGIC 自定义 AVP 的全局句柄结构，
 *          以及所有协议常量（命令码、AVP 码）。供客户端快速访问词典对象。
 */
 ...
 * 基础协议常量
 * ========================================================================== */
#define MAGIC_VENDOR_ID 13712   ///< AEEC Vendor ID (ARINC 839)
#define MAGIC_APP_ID 1094202169 ///< MAGIC Auth-Application-ID
...
/* ==========================================================================
 * 标准 Diameter 核心 AVP 句柄结构体（Vendor=0）
 * ========================================================================== */
/**
 * @brief 标准 Diameter Base Protocol AVP 句柄集合。
 * @details 缓存 RFC 6733 定义的常用 AVP 对象指针，避免重复查询词典。
 */
struct std_diam_dict_handles {
  struct dict_object *avp_session_id;          ///< Session-Id (263)
  struct dict_object *avp_origin_host;         ///< Origin-Host (264)
  struct dict_object *avp_origin_realm;        ///< Origin-Realm (296)
  struct dict_object *avp_destination_host;    ///< Destination-Host (293)
  struct dict_object *avp_destination_realm;   ///< Destination-Realm (283)
  struct dict_object *avp_auth_application_id; ///< Auth-Application-Id (258)
  struct dict_object *avp_result_code;         ///< Result-Code (268)
  struct dict_object
      *avp_experimental_result_code; ///< Experimental-Result-Code (298)
  struct dict_object *avp_user_name; ///< User-Name (1)
...
/* ==========================================================================
 * MAGIC 自定义协议专属句柄结构体（Vendor=13712）
 * ========================================================================== */
/**
 * @brief MAGIC 协议 AVP 和命令句柄集合。
 * @details 缓存所有 ARINC 839 定义的自定义 AVP、命令及分组 AVP 对象指针。
 */
struct magic_dict_handles {
  struct dict_object *vendor; ///< AEEC Vendor 对象 (13712)
  struct dict_object *app;    ///< MAGIC Application 对象 (1094202169)

  /* ==================== 7个自定义命令 ==================== */
  struct dict_object *cmd_mcar; // 350：客户端认证请求/应答
  struct dict_object *cmd_mccr; // 351：通信参数变更请求/应答
  struct dict_object *cmd_mntr; // 352：通知报告请求/应答
  struct dict_object *cmd_mscr; // 353：状态变更报告请求/应答
  struct dict_object *cmd_msxr; // 354：状态查询请求/应答
  struct dict_object *cmd_madr; // 355：计费数据请求/应答
  struct dict_object *cmd_macr; // 356：计费控制请求/应答
  ...
  struct dict_object *avp_dlm_link_status_list; // 20010：DLM下所有链路状态列表
 ...
 * ========================================================================== */
/**
 * @brief 标准 Diameter 字典句柄全局实例。
 * @note 定义在 magic_dict_handles.c 中。
 */
extern struct std_diam_dict_handles g_std_dict;

/**
 * @brief MAGIC 自定义协议字典句柄全局实例。
 * @note 定义在 magic_dict_handles.c 中。
 */
...
/**
 * @brief 初始化所有词典句柄。
 * @details 从 freeDiameter 全局词典中查询并缓存所有 AVP/命令对象指针。
 *          必须在 freeDiameter 核心初始化和字典加载后调用。
 *
 * @return int 成功返回 0，失败返回非零错误码。
 * @warning 必须在使用任何 AVP 句柄前调用此函数。
 */
