 ...
 * @brief MAGIC 数据平面路由模块实现 (ARINC 839 合规版)。
 * @details 采用基于标记的路由 (Mark Based Routing) 策略。通过 Linux iptables
 * 对匹配 TFT 的流量打上 FwMark， 并利用策略路由将其引流至对应的链路出口。
 */
...
int magic_dataplane_ipset_init(DataplaneContext *ctx) {
  ...
  /* 创建 control 集合 (hash:ip) */
  snprintf(cmd, sizeof(cmd),
           "ipset create magic_control hash:ip family inet -exist");
  if (magic_dataplane_exec_cmd(cmd) != 0) {
    fd_log_error("[dataplane] ipset create magic_control 失败");
    /* 继续尝试创建 data 集合 */
  } else {
    fd_log_notice("[dataplane] ✓ ipset: magic_control 已准备");
  }

  /* 创建 data 集合 */
  snprintf(cmd, sizeof(cmd),
           "ipset create magic_data hash:ip family inet -exist");
  if (magic_dataplane_exec_cmd(cmd) != 0) {
    fd_log_error("[dataplane] ipset create magic_data 失败");
    return -1;
  }

  fd_log_notice("[dataplane] ✓ ipset: magic_data 已准备");
  return 0;
}
...
int magic_dataplane_ipset_del(const char *client_ip) {
  ...
  snprintf(cmd, sizeof(cmd), "ipset del magic_control %s 2>/dev/null",
           client_ip);
  magic_dataplane_exec_cmd(cmd);
 ...
 * @return 0 成功, -1 失败
 */
static int get_interface_ip(const char *interface_name, char *ip_buf,
                            size_t buf_len) {
  if (!interface_name || !ip_buf || buf_len == 0)
    return -1;
 ...
 */
static int count_client_ip_sessions(DataplaneContext *ctx,
 ...
 */
static LinkRouteConfig *find_link_config(DataplaneContext *ctx,
                                         const char *link_id) {
...
static int create_route_table(LinkRouteConfig *link) {
  ...
  /* 清理可能存在的旧路由 */
  snprintf(cmd, sizeof(cmd), "ip route flush table %u 2>/dev/null",
           link->route_table_id);
  magic_dataplane_exec_cmd(cmd);

  /* 检查网关 IP 是否等于接口 IP（测试环境：服务器和DLM在同一机器） */
  if (link->gateway_ip[0]) {
    if (get_interface_ip(link->interface_name, interface_ip,
                         sizeof(interface_ip)) == 0) {
      if (strcmp(link->gateway_ip, interface_ip) == 0) {
        fd_log_notice(
            "[dataplane] 网关 IP (%s) = 接口 IP (测试环境)，使用直连路由",
            link->gateway_ip);
  ...
  if (link->gateway_ip[0] && !use_direct_route) {
    ...
    snprintf(cmd, sizeof(cmd), "ip route add default dev %s table %u",
  ...
  if (ret != 0) {
    ...
    fd_log_error("[dataplane]   接口=%s, 网关=%s, 表=%u", link->interface_name,
...
static int delete_route_table(LinkRouteConfig *link) {
  ...
  fd_log_notice("[dataplane] 删除路由表 %u: %s", link->route_table_id,
                link->link_id);
...
/*===========================================================================
 * IP Rule 管理 (废弃: 旧的基于源 IP 的路由方式)
 * 保留用于向后兼容，新代码应使用 TFT 规则
 *===========================================================================*/

/**
 * @brief 添加 IP 策略规则 (废弃)
 * @deprecated 使用 magic_dataplane_add_tft_rule 替代
 */
__attribute__((unused)) static int
add_ip_rule(const char *client_ip, uint32_t table_id, uint32_t priority) {
  char cmd[MAX_CMD_LEN];

  snprintf(cmd, sizeof(cmd), "ip rule add from %s lookup %u priority %u",
           client_ip, table_id, priority);

  int ret = magic_dataplane_exec_cmd(cmd);
  if (ret != 0) {
    fd_log_error("[dataplane] 添加 ip rule 失败: from %s lookup %u", client_ip,
                 table_id);
    return -1;
  }
...
/**
 * @brief 删除 IP 策略规则 (废弃)
 * @deprecated 使用 magic_dataplane_remove_tft_rules 替代
 */
__attribute__((unused)) static int delete_ip_rule(const char *client_ip,
                                                  uint32_t table_id) {
  char cmd[MAX_CMD_LEN];

  snprintf(cmd, sizeof(cmd), "ip rule del from %s lookup %u 2>/dev/null",
           client_ip, table_id);

  int ret = magic_dataplane_exec_cmd(cmd);
  if (ret != 0) {
    fd_log_debug(
        "[dataplane] 删除 ip rule 失败 (可能不存在): from %s lookup %u",
        client_ip, table_id);
    return -1;
  }

  fd_log_notice("[dataplane] ✓ 删除 ip rule: from %s lookup %u", client_ip,
                table_id);
 ...
 * 当会话建立时，使用 iptables mangle 表对匹配 TFT 的流量打 fwmark
 * 然后通过静态 ip rule 规则将流量路由到对应链路
 *
 * ARINC 839 合规实现原理:
 * - 初始化时创建静态 fwmark 路由规则 (ip rule add fwmark N lookup N)
 * - 会话建立时: 添加 mangle PREROUTING 打标规则 + filter FORWARD 放行规则
 * - 会话关闭时: 删除对应的 mangle 和 filter 规则
 * - 使用黑洞路由表 (table 99) 用于未授权流量
 *===========================================================================*/

#define MAGIC_BLACKHOLE_TABLE MAGIC_FWMARK_BLACKHOLE

/**
 * @brief 初始化 fwmark 静态路由规则
 * 在系统启动时创建，运行期间不变
 *
 * 创建规则: ip rule add fwmark N lookup N priority 100
 * fwmark 范围: 100-199
 *
 * @return 0=成功, -1=失败
 */
static int init_fwmark_rules(DataplaneContext *ctx) {
  ...
  fd_log_notice("[dataplane] 初始化 fwmark 静态路由规则...");

  /* 创建 fwmark 100-199 的静态规则 */
  for (uint32_t mark = MAGIC_RT_TABLE_BASE; mark <= MAGIC_RT_TABLE_MAX;
       mark++) {
    /* 先删除可能存在的旧规则 */
    snprintf(cmd, sizeof(cmd), "ip rule del fwmark %u lookup %u 2>/dev/null",
             mark, mark);
    magic_dataplane_exec_cmd(cmd);

    /* 添加新规则 */
    snprintf(cmd, sizeof(cmd), "ip rule add fwmark %u lookup %u priority %u",
             mark, mark, MAGIC_FWMARK_RULE_PRIORITY);
    if (magic_dataplane_exec_cmd(cmd) == 0) {
      installed++;
    }
  }
  ...
  fd_log_notice(
      "[dataplane] ✓ 已安装 %d 条 fwmark 路由规则 (mark 100-%u → table 100-%u)",
      installed, MAGIC_RT_TABLE_MAX, MAGIC_RT_TABLE_MAX);
...
/**
 * @brief 初始化基于 fwmark 的 NAT 规则
 * 为每个 fwmark 值创建对应的 MASQUERADE 规则
 *
 * @return 0=成功, -1=失败
 */
static int init_mark_based_nat(DataplaneContext *ctx) {
  char cmd[MAX_CMD_LEN];
  (void)ctx;

  fd_log_notice("[dataplane] 初始化基于 fwmark 的 NAT 规则...");

  /* 为每个可能的 fwmark 值创建 NAT 规则 */
  for (uint32_t mark = MAGIC_RT_TABLE_BASE; mark <= MAGIC_RT_TABLE_MAX;
       mark++) {
    /* 先删除可能存在的旧规则 */
    snprintf(cmd, sizeof(cmd),
             "iptables -t nat -D POSTROUTING -m mark --mark %u -j MASQUERADE "
             "2>/dev/null",
             mark);
    magic_dataplane_exec_cmd(cmd);
  ...
  fd_log_notice("[dataplane] ✓ 已创建基于 fwmark 的 NAT 规则 (mark 100-%u)",
                MAGIC_RT_TABLE_MAX);
  return 0;
}
 ...
 * 创建一个专用路由表，将所有流量丢弃
 */
static int init_blackhole_table(void) {
  char cmd[MAX_CMD_LEN];

  /* 清理可能存在的旧配置 */
  snprintf(cmd, sizeof(cmd), "ip route flush table %d 2>/dev/null",
           MAGIC_BLACKHOLE_TABLE);
  magic_dataplane_exec_cmd(cmd);

  /* 添加黑洞路由 - 所有流量都被丢弃 */
  snprintf(cmd, sizeof(cmd), "ip route add blackhole default table %d",
           MAGIC_BLACKHOLE_TABLE);
  int ret = magic_dataplane_exec_cmd(cmd);
  if (ret != 0) {
  ...
  fd_log_notice("[dataplane] ✓ 初始化黑洞路由表 (table=%d)",
                MAGIC_BLACKHOLE_TABLE);
  return 0;
}
 ...
 */
static int remove_client_iptables_rules(const char *client_ip,
   ...
   * - iptables ... -s <client_ip> -d <any_dest> -p <proto> --dport <port> -j
   * ACCEPT 不管目的IP/端口/协议是什么，只要源IP匹配就删除
   */

  /* 删除 OUTPUT 链中所有源为 client_ip 的规则（任意目的地） */
  snprintf(cmd, sizeof(cmd),
           "iptables-save | grep -v -- '-A OUTPUT.*-s %s' | iptables-restore "
           "2>/dev/null",
           client_ip);
  magic_dataplane_exec_cmd(cmd);
  ...
  snprintf(cmd, sizeof(cmd),
           "iptables-save | grep -v -- '-A FORWARD.*-s %s' | iptables-restore "
           "2>/dev/null",
           client_ip);
  magic_dataplane_exec_cmd(cmd);
  ...
  snprintf(cmd, sizeof(cmd),
           "iptables-save | grep -v -- '-A FORWARD.*-d %s' | iptables-restore "
           "2>/dev/null",
           client_ip);
  magic_dataplane_exec_cmd(cmd);

  fd_log_notice("[dataplane] ✓ 清理所有 iptables 规则: %s (包括精确目的IP规则)",
                client_ip);
  return 0;
}
 ...
 */
static int remove_blackhole_rule(const char *client_ip,
                                 const char *gateway_ip) {
  char cmd[MAX_CMD_LEN];
  ...
  /* 删除可能存在的黑洞路由规则 */
  snprintf(cmd, sizeof(cmd),
           "ip rule del from %s lookup %d priority %d 2>/dev/null", client_ip,
           MAGIC_BLACKHOLE_TABLE, MAGIC_BLACKHOLE_PRIORITY);
  magic_dataplane_exec_cmd(cmd);
...
/**
 * @brief 设置客户端的链路访问规则 (废弃)
 * @deprecated 使用 TFT 规则替代，此函数保留用于向后兼容
 *
 * 实现精确的流量控制：
 ...
 */
__attribute__((unused)) static int
setup_client_link_access(const char *client_ip, const char *gateway_ip,
                         const char *dest_ip) {
  char cmd[MAX_CMD_LEN];
  ...
  snprintf(cmd, sizeof(cmd), "iptables -I OUTPUT 1 -s %s -d %s -j ACCEPT",
           client_ip, target);
  int ret = magic_dataplane_exec_cmd(cmd);
  if (ret == 0) {
  ...
  /* 同时在 FORWARD 链插入对应的 ACCEPT，允许被路由/转发的客户端流量 */
  snprintf(cmd, sizeof(cmd), "iptables -I FORWARD 1 -s %s -d %s -j ACCEPT",
           client_ip, target);
  magic_dataplane_exec_cmd(cmd);
  fd_log_notice("[dataplane] ✓ iptables FORWARD: %s → %s", client_ip, target);
...
/**
 * @brief 添加客户端的黑洞规则，阻断所有流量 (废弃)
 * @deprecated 使用 TFT 规则替代，删除规则后默认阻断
 *
 * 会话关闭时调用：
 * 1. 删除该客户端的 ACCEPT 规则（使网段 DROP 规则生效）
 * 2. 添加 ip rule 黑洞路由（用于外部流量）
 */
__attribute__((unused)) static int add_blackhole_rule(const char *client_ip,
                                                      const char *gateway_ip) {
  char cmd[MAX_CMD_LEN];
  ...
  /* 1. 添加黑洞路由规则 (用于外部流量) */
  snprintf(cmd, sizeof(cmd), "ip rule add from %s lookup %d priority %d",
  ...
  fd_log_notice("[dataplane] ✓ 添加黑洞规则: %s → blackhole (流量已阻断)",
                client_ip);
  return 0;
}
...
int magic_dataplane_init(DataplaneContext *ctx, const char *ingress_if,
  ...
  init_mark_based_nat(ctx);

  /* === 第三步：初始化黑洞路由表 === */
  init_blackhole_table();

  /* === 第四步：清理所有残留的客户端规则 === */
  fd_log_notice("[dataplane] 清理客户端网段残留规则...");

  /* 清理 iptables mangle 表中的旧规则 */
  magic_dataplane_exec_cmd(
      "iptables -t mangle -F PREROUTING 2>/dev/null || true");

  /* 清理 iptables filter 表中的旧规则 */
  magic_dataplane_exec_cmd("iptables-save | grep -v '192\\.168\\.126\\.' | "
                           "iptables-restore 2>/dev/null || true");

  /* 清理所有客户端 IP 的连接跟踪条目 */
  magic_dataplane_exec_cmd(
      "conntrack -D -s 192.168.126.0/24 2>/dev/null || true");

  fd_log_notice("[dataplane] ✓ 客户端网段残留规则已清理");

  /* === 第五步：设置连接跟踪规则 (ESTABLISHED,RELATED 优先放行) === */
  fd_log_notice("[dataplane] 设置连接跟踪规则...");

  /* 清理旧的连接跟踪规则 */
  magic_dataplane_exec_cmd("iptables -D FORWARD -m conntrack --ctstate "
                           "ESTABLISHED,RELATED -j ACCEPT 2>/dev/null");

  /* 在 FORWARD 链最前面添加连接跟踪规则 */
  magic_dataplane_exec_cmd("iptables -I FORWARD 1 -m conntrack --ctstate "
                           "ESTABLISHED,RELATED -j ACCEPT");
  fd_log_notice("[dataplane] ✓ 连接跟踪规则: ESTABLISHED,RELATED → ACCEPT");

  /* === 第六步：设置默认阻断规则 === */
  fd_log_notice("[dataplane] 设置默认流量阻断规则...");

  /* FORWARD 链：阻断未授权的客户端流量 */
  magic_dataplane_exec_cmd(
      "iptables -D FORWARD -s 192.168.126.0/24 -j DROP 2>/dev/null");
  ...
  fd_log_notice("[dataplane]   入口接口: %s",
                ingress_if ? ingress_if : "未指定");
  fd_log_notice("[dataplane]   入口 IP: %s",
                ingress_ip ? ingress_ip : "未指定");
  fd_log_notice("[dataplane]   路由模式: Mark Based Routing (fwmark)");
  fd_log_notice("[dataplane]   流量控制: mangle 表打标 + filter 表放行");
  fd_log_notice("[dataplane]   fwmark 规则: %s",
                ctx->fwmark_rules_installed ? "已安装" : "未安装");
...
int magic_dataplane_register_link(DataplaneContext *ctx, const char *link_id,
  ...
  if (existing) {
    fd_log_notice("[dataplane] 链路已注册: %s (table=%u)", link_id,
                  existing->route_table_id);
  ...
  /* 分配路由表 ID 和 fwmark (相同值) */
  uint32_t table_id = MAGIC_RT_TABLE_BASE + ctx->num_links;
  uint32_t fwmark = table_id; /* fwmark 与 table_id 相同 */
  ...
  if (create_route_table(link) != 0) {
    fd_log_notice("[dataplane] ⚠ 路由表创建失败（稍后可通过 DLM 更新）: %s",
                  link_id);
  ...
  fd_log_notice(
      "[dataplane] ✓ 注册链路: %s → %s (table=%u, fwmark=%u, configured=%s)",
      link_id, interface_name, table_id, fwmark,
...
/*===========================================================================
 * TFT (Traffic Flow Template) 规则管理 - ARINC 839 合规
 * 基于五元组的精确流量控制，使用 mangle 表打 fwmark
 *===========================================================================*/

/**
 * @brief 查找空闲的 TFT 规则槽位
 */
static TftRule *find_free_tft_slot(DataplaneContext *ctx) {
  for (uint32_t i = 0; i < MAX_TFT_RULES; i++) {
    if (!ctx->tft_rules[i].in_use) {
      return &ctx->tft_rules[i];
    }
  }
  return NULL;
}
...
/**
 * @brief 构建 TFT 匹配条件字符串
 * 生成 iptables 匹配条件: -s <src_ip> -d <dst_ip> -p <proto> --sport <src_port>
 * --dport <dst_port>
 */
static int build_tft_match(const TftTuple *tuple, char *match_str,
                           size_t match_len) {
  if (!tuple || !match_str || match_len == 0)
    return -1;
  ...
  /* 源 IP (必须) */
  if (tuple->src_ip[0]) {
    int n = snprintf(match_str + offset, match_len - offset, "-s %s ",
                     tuple->src_ip);
    if (n > 0)
      offset += n;
  }
  ...
  /* 目的 IP (可选) */
  if (tuple->dst_ip[0]) {
    int n = snprintf(match_str + offset, match_len - offset, "-d %s ",
                     tuple->dst_ip);
    if (n > 0)
      offset += n;
  }

  /* 协议 (可选: 6=TCP, 17=UDP, 1=ICMP, 0=任意) */
  if (tuple->protocol > 0) {
    const char *proto_name = "all";
    if (tuple->protocol == 6)
      proto_name = "tcp";
    else if (tuple->protocol == 17)
      proto_name = "udp";
    else if (tuple->protocol == 1)
      proto_name = "icmp";

    int n =
        snprintf(match_str + offset, match_len - offset, "-p %s ", proto_name);
    if (n > 0)
      offset += n;

    /* 源端口 (仅 TCP/UDP) */
    if (tuple->src_port > 0 &&
        (tuple->protocol == 6 || tuple->protocol == 17)) {
      n = snprintf(match_str + offset, match_len - offset, "--sport %u ",
                   tuple->src_port);
      if (n > 0)
        offset += n;
    }
    ...
    if (tuple->dst_port > 0 &&
        (tuple->protocol == 6 || tuple->protocol == 17)) {
      n = snprintf(match_str + offset, match_len - offset, "--dport %u ",
                   tuple->dst_port);
      if (n > 0)
        offset += n;
    }
...
/**
 * @brief 添加 TFT 规则
 * 添加基于五元组的 mangle PREROUTING 打标规则和 filter FORWARD 放行规则
 */
int magic_dataplane_add_tft_rule(DataplaneContext *ctx, const TftTuple *tuple,
  ...
  if (!link) {
    fd_log_error("[dataplane] TFT 规则: 链路未注册: %s", link_id);
    pthread_mutex_unlock(&ctx->mutex);
    return -1;
  }

  /* 查找空闲槽位 */
  TftRule *rule = find_free_tft_slot(ctx);
  if (!rule) {
  ...
  uint32_t fwmark = link->fwmark;
  char match_str[256] = {0};
  char cmd[MAX_CMD_LEN];

  /* 构建 TFT 匹配条件 */
  build_tft_match(tuple, match_str, sizeof(match_str));

  /* 1. 添加 mangle PREROUTING 打标规则 */
  snprintf(cmd, sizeof(cmd),
           "iptables -t mangle -A PREROUTING %s -j MARK --set-mark %u",
           match_str, fwmark);
  if (magic_dataplane_exec_cmd(cmd) != 0) {
    fd_log_error("[dataplane] TFT mangle 规则添加失败: %s", match_str);
    pthread_mutex_unlock(&ctx->mutex);
    return -1;
  }
  fd_log_notice("[dataplane] ✓ TFT mangle: %s → fwmark %u", match_str, fwmark);

  /* 2. 添加 filter FORWARD 放行规则 (精确五元组) */
  snprintf(cmd, sizeof(cmd), "iptables -I FORWARD 2 %s -j ACCEPT", match_str);
  if (magic_dataplane_exec_cmd(cmd) != 0) {
    fd_log_error("[dataplane] TFT filter 规则添加失败: %s", match_str);
    /* 回滚 mangle 规则 */
    snprintf(
        cmd, sizeof(cmd),
        "iptables -t mangle -D PREROUTING %s -j MARK --set-mark %u 2>/dev/null",
        match_str, fwmark);
    magic_dataplane_exec_cmd(cmd);
    pthread_mutex_unlock(&ctx->mutex);
    return -1;
  }
  fd_log_notice("[dataplane] ✓ TFT filter: %s → ACCEPT", match_str);

  /* 3. 添加反向流量规则 (回程) */
  if (tuple->dst_ip[0]) {
    snprintf(cmd, sizeof(cmd), "iptables -I FORWARD 2 -s %s -d %s -j ACCEPT",
             tuple->dst_ip, tuple->src_ip);
    magic_dataplane_exec_cmd(cmd);
    fd_log_notice("[dataplane] ✓ TFT 回程: %s → %s ACCEPT", tuple->dst_ip,
                  tuple->src_ip);
  }
  ...
  fd_log_notice(
      "[dataplane] ✓ TFT 规则添加成功: session=%s, link=%s, fwmark=%u",
      session_id, link_id, fwmark);
...
int magic_dataplane_remove_tft_rules(DataplaneContext *ctx,
  ...
  for (uint32_t i = 0; i < MAX_TFT_RULES; i++) {
    ...
    if (rule->in_use && strcmp(rule->session_id, session_id) == 0) {
      /* 构建匹配条件 */
      build_tft_match(&rule->tuple, match_str, sizeof(match_str));

      /* 删除 mangle 规则 */
      snprintf(cmd, sizeof(cmd),
               "iptables -t mangle -D PREROUTING %s -j MARK --set-mark %u "
               "2>/dev/null",
               match_str, rule->fwmark);
      magic_dataplane_exec_cmd(cmd);

      /* 删除 filter 规则 */
      snprintf(cmd, sizeof(cmd), "iptables -D FORWARD %s -j ACCEPT 2>/dev/null",
               match_str);
      magic_dataplane_exec_cmd(cmd);

      /* 删除回程规则 */
      if (rule->tuple.dst_ip[0]) {
        snprintf(cmd, sizeof(cmd),
                 "iptables -D FORWARD -s %s -d %s -j ACCEPT 2>/dev/null",
                 rule->tuple.dst_ip, rule->tuple.src_ip);
        magic_dataplane_exec_cmd(cmd);
      }

      fd_log_notice("[dataplane] ✓ 删除 TFT 规则: session=%s, fwmark=%u",
                    session_id, rule->fwmark);
...
int magic_dataplane_find_tft_rules(DataplaneContext *ctx,
  ...
  for (uint32_t i = 0; i < MAX_TFT_RULES && found < max_rules; i++) {
    if (ctx->tft_rules[i].in_use &&
        strcmp(ctx->tft_rules[i].session_id, session_id) == 0) {
      rules[found++] = &ctx->tft_rules[i];
    }
  }
 ...
 */
uint32_t magic_dataplane_get_fwmark(DataplaneContext *ctx,
                                    const char *link_id) {
  if (!ctx || !link_id)
    return 0;
...
int magic_dataplane_add_client_route(DataplaneContext *ctx,
  ...
  strncpy(tuple.src_ip, client_ip, MAX_IP_ADDR_LEN - 1);
  if (dest_ip && dest_ip[0]) {
    strncpy(tuple.dst_ip, dest_ip, MAX_IP_ADDR_LEN - 1);
  }
  /* 默认协议为 0 (任意), 端口为 0 (任意) */
  tuple.protocol = 0;
  tuple.src_port = 0;
  ...
  /* 删除旧的 TFT 规则 */
  magic_dataplane_remove_tft_rules(ctx, session_id);

  /* 添加新的 TFT 规则 */
  int ret = magic_dataplane_add_tft_rule(ctx, &tuple, session_id, link_id);
  if (ret != 0) {
    fd_log_error("[dataplane] 添加客户端路由失败: %s → %s", client_ip, link_id);
    return -1;
  }

  /* 同时记录到旧的 rules 数组 (兼容性) */
  pthread_mutex_lock(&ctx->mutex);
  ...
  fd_log_notice("[dataplane] ✓ 客户端路由: %s → %s (fwmark=%u)", client_ip,
                link_id, link ? link->fwmark : 0);
...
int magic_dataplane_remove_client_route(DataplaneContext *ctx,
  ...
  /* 删除 TFT 规则 */
  int removed = magic_dataplane_remove_tft_rules(ctx, session_id);
  if (removed == 0) {
    fd_log_debug("[dataplane] 未找到会话的 TFT 规则: %s", session_id);
  }
  ...
  if (rule) {
    ...
    fd_log_notice("[dataplane] ✓ 删除客户端路由: %s → %s (fwmark=%u)",
                  rule->client_ip, rule->link_id, rule->fwmark);

    rule->in_use = false;
    ctx->num_rules--;

    /* ★★★ 关键修复：检查该 IP 是否还有其他活跃会话 ★★★
     * 如果还有其他会话，不能清理 iptables/ipset 规则，否则会阻断其他会话的流量
     */
    int other_sessions = count_client_ip_sessions(ctx, client_ip, NULL);
    bool should_cleanup = (other_sessions == 0);

    if (!should_cleanup) {
      fd_log_notice(
          "[dataplane] ⚠ 该 IP (%s) 还有 %d 个其他活跃会话，保留网络规则",
          client_ip, other_sessions);
    }

    pthread_mutex_unlock(&ctx->mutex);

    /* 只有在没有其他会话时才清理 ipset 和 conntrack */
    if (should_cleanup) {
      /* 从 ipset 中删除客户端 */
...
int magic_dataplane_switch_client_link(DataplaneContext *ctx,
  ...
  ClientRoutingRule *rule = magic_dataplane_find_rule(ctx, session_id);
  if (!rule) {
    fd_log_error("[dataplane] 未找到会话的路由规则: %s", session_id);
    pthread_mutex_unlock(&ctx->mutex);
    return -1;
  }
  ...
  if (strcmp(rule->link_id, new_link_id) == 0) {
    fd_log_debug("[dataplane] 链路未变化: %s", new_link_id);
    pthread_mutex_unlock(&ctx->mutex);
    return 0;
  }

  char old_link_id[MAX_LINK_ID_LEN];
  char client_ip[MAX_IP_ADDR_LEN];
  strncpy(old_link_id, rule->link_id, MAX_LINK_ID_LEN - 1);
  strncpy(client_ip, rule->client_ip, MAX_IP_ADDR_LEN - 1);
  ...
  /* 构建新的 TFT 五元组 (使用客户端 IP) */
  TftTuple tuple;
  memset(&tuple, 0, sizeof(tuple));
  strncpy(tuple.src_ip, client_ip, MAX_IP_ADDR_LEN - 1);

  /* 添加新的 TFT 规则 */
  if (magic_dataplane_add_tft_rule(ctx, &tuple, session_id, new_link_id) != 0) {
    fd_log_error("[dataplane] 切换链路失败: %s → %s", session_id, new_link_id);
    /* 尝试恢复旧规则 */
    magic_dataplane_add_tft_rule(ctx, &tuple, session_id, old_link_id);
    return -1;
  }

  /* 更新旧 API 规则记录 */
  pthread_mutex_lock(&ctx->mutex);
  rule = magic_dataplane_find_rule(ctx, session_id);
  if (rule) {
    strncpy(rule->link_id, new_link_id, MAX_LINK_ID_LEN - 1);
    rule->route_table_id = new_link->route_table_id;
    rule->fwmark = new_link->fwmark;
  }
  pthread_mutex_unlock(&ctx->mutex);

  fd_log_notice("[dataplane] ✓ 切换链路: %s: %s → %s (fwmark=%u)", client_ip,
                old_link_id, new_link_id, new_link->fwmark);
...
int magic_dataplane_get_link_gateway(DataplaneContext *ctx, const char *link_id,
  ...
  if (!ctx || !link_id || !gateway_ip || ip_len == 0)
    return -1;
...
int magic_dataplane_unregister_link(DataplaneContext *ctx,
                                    const char *link_id) {
  if (!ctx || !ctx->is_initialized) {
    fd_log_error("[dataplane] 数据平面未初始化");
    return -1;
  ...
  if ((uint32_t)link_index < ctx->num_links - 1) {
    memcpy(&ctx->links[link_index], &ctx->links[ctx->num_links - 1],
           sizeof(LinkRouteConfig));
  }
  ctx->num_links--;
...
int magic_dataplane_update_link_gateway(DataplaneContext *ctx,
  ...
  if (gateway_ip && gateway_ip[0]) {
    ...
    if (create_route_table(link) != 0) {
      fd_log_notice("[dataplane] ⚠ 路由表创建失败（网关可能不可达），但 "
                    "Gateway IP 已保存: %s -> %s",
                    link_id, link->gateway_ip);
      /* 不返回错误，gateway IP 已保存供客户端使用 */
    } else {
      fd_log_notice("[dataplane] ✓ 更新链路网关并重建路由表: %s -> %s", link_id,
                    link->gateway_ip);
    }
  } else {
    /* 如果没有提供网关，仅记录 */
...
void magic_dataplane_print_status(DataplaneContext *ctx) {
  ...
  fd_log_notice("[dataplane] ════════════════════════════════════════");
  fd_log_notice("[dataplane] 数据平面状态 (ARINC 839 合规)");
  fd_log_notice("[dataplane] ════════════════════════════════════════");
  ...
  fd_log_notice("[dataplane] fwmark 规则: %s",
                ctx->fwmark_rules_installed ? "已安装" : "未安装");
  fd_log_notice("[dataplane] 路由功能: %s",
                ctx->enable_routing ? "启用" : "禁用");
  ...
  fd_log_notice("[dataplane] ─────────────────────────────────────");
  fd_log_notice("[dataplane] TFT 规则 (%u):", ctx->num_tft_rules);
  for (uint32_t i = 0; i < MAX_TFT_RULES; i++) {
    if (ctx->tft_rules[i].in_use) {
      TftRule *rule = &ctx->tft_rules[i];
      fd_log_notice("[dataplane]   %s:%u → %s:%u (proto=%u) → %s (fwmark=%u)",
                    rule->tuple.src_ip, rule->tuple.src_port,
                    rule->tuple.dst_ip[0] ? rule->tuple.dst_ip : "*",
                    rule->tuple.dst_port, rule->tuple.protocol, rule->link_id,
                    rule->fwmark);
    }
  }

  fd_log_notice("[dataplane] ─────────────────────────────────────");
  fd_log_notice("[dataplane] 客户端规则 (%u):", ctx->num_rules);
  for (uint32_t i = 0; i < MAX_ROUTING_RULES; i++) {
    if (ctx->rules[i].in_use) {
      ClientRoutingRule *rule = &ctx->rules[i];
      fd_log_notice("[dataplane]   %s → %s (fwmark=%u)", rule->client_ip,
...
void magic_dataplane_cleanup(DataplaneContext *ctx) {
  ...
  /* 清理 mangle 表 PREROUTING 链 */
  magic_dataplane_exec_cmd("iptables -t mangle -F PREROUTING 2>/dev/null");

  /* 删除连接跟踪规则 */
  magic_dataplane_exec_cmd("iptables -D FORWARD -m conntrack --ctstate "
                           "ESTABLISHED,RELATED -j ACCEPT 2>/dev/null");

  /* 删除 ipset 相关的 iptables 规则 */
  magic_dataplane_exec_cmd("iptables -D FORWARD -m set --match-set magic_data "
                           "src -j ACCEPT 2>/dev/null");
  magic_dataplane_exec_cmd("iptables -D FORWARD -m set --match-set "
                           "magic_control src -j ACCEPT 2>/dev/null");
