 ...
 * @brief MAGIC 协议字典句柄管理器实现。
 * @details 负责查询并缓存标准 Diameter 和 MAGIC 自定义 AVP 的词典对象句柄，
 *          避免运行时重复查询，提升消息构造性能。
 */
...
/**
 * @brief 标准 Diameter AVP 句柄全局实例。
 */
struct std_diam_dict_handles g_std_dict = {0};

/**
 * @brief MAGIC 自定义协议句柄全局实例。
 */
struct magic_dict_handles g_magic_dict = {0};

/**
 * @brief 辅助宏：查找标准 Diameter AVP（Vendor=0）。
 * @param code AVP 代码。
 * @param target 存储查询结果的指针变量。
 */
...
/**
 * @brief 辅助宏：查找 MAGIC 自定义 AVP（Vendor=13712）。
 * @param code AVP 代码。
 * @param target 存储查询结果的指针变量。
 */
...
/**
 * @brief 初始化所有词典句柄。
 * @details 从 freeDiameter 全局字典中查找并填充：
 *          1. Vendor 和 Application 对象
 *          2. 标准 Diameter 核心 AVP (Vendor=0)
 *          3. MAGIC 自定义命令 (100000-100006)
 *          4. MAGIC 自定义普通 AVP (10001-10054)
 *          5. MAGIC 自定义分组 AVP (20001-20019)
 *
 * @return int 成功返回 0，失败返回 freeDiameter 错误码。
 *
 * @warning 必须在 freeDiameter 核心初始化完成后调用。
 * @note 如果任何必须的 AVP 未找到，函数将返回 ENOENT。
 */
