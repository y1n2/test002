/**
 * ============================================================================
 * @file    dlm_cellular_standard.c
 * @brief   CELLULAR 数据链路管理器 (DLM) - Standard MIH 实现
 *
 * @details 遵循 ARINC 839-2014 MIH 协议规范的 CELLULAR 链路标准实现
...
static void *dlm_packet_monitor_thread(void *arg) {
  ...
  printf("[CELLULAR-PKT] Packet monitor thread started, iface=%s\n",
         g_ctx.config.interface_name);

  int pipefd[2];
  if (pipe(pipefd) != 0) {
    printf("[CELLULAR-PKT] ERROR: pipe() failed: %s\n", strerror(errno));
    return NULL;
  }

  pid_t pid = fork();
  if (pid < 0) {
    printf("[CELLULAR-PKT] ERROR: fork() failed: %s\n", strerror(errno));
    close(pipefd[0]);
    close(pipefd[1]);
    return NULL;
  }
  ...
  if (!fp) {
    printf("[CELLULAR-PKT] ERROR: fdopen() failed: %s\n", strerror(errno));
    kill(pid, SIGINT);
  ...
  printf("[CELLULAR-PKT] tcpdump started (pid=%d)\n", (int)pid);
  ...
  printf("[CELLULAR-PKT] Packet monitor thread exit\n");
  return NULL;
}
...
static int dlm_init_config_manager(const char *config_path) {
  ...
  printf("[CELLULAR-CM] 配置管理器初始化完成 (Standard MIH Mode)\n");
  return 0;
}
...
static int dlm_connect_to_server(void) {
  ...
  printf("[CELLULAR] Connecting to MIH Server at %s ...\n",
         MIH_STANDARD_SOCKET_PATH);

  if (connect(g_ctx.socket_fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    perror("[CELLULAR] connect() failed");
    return -1;
  }

  printf("[CELLULAR] Connected! Sending Registration...\n");
...
static int dlm_send_mih_message(uint16_t type, const void *payload,
  ...
  if (sizeof(mih_transport_header_t) + payload_len > sizeof(buffer)) {
    fprintf(stderr, "[CELLULAR] Message too large\n");
    return -1;
  }
...
static int dlm_send_register_request(void) {
  ...
  printf("[CELLULAR] Sending MIH_EXT_Link_Register (0x8101)...\n");
 ...
 *
 * 周期性调用，向 CM Core 汇报当前实时的链路质量指标
 * 使用标准 MIH 原语 MIH_LINK_PARAMETERS_REPORT_IND
 */
static int dlm_send_parameters_report(void) {
  ...
  /* 1. 设置链路标识符 (Link Identifier) */
  ind.link_identifier.link_type =
      g_ctx.config.link_id; /* 链路类型 (如 LINK_TYPE_CELLULAR) */
  strncpy(ind.link_identifier.link_addr, g_ctx.config.link_name,
          sizeof(ind.link_identifier.link_addr) - 1); /* 链路名称/地址 */

  /* 2. 设置参数位图 (标记哪些参数已变化，此处全选) */
...
static void *dlm_message_receiver_thread(void *arg) {
  ...
  printf("[CELLULAR-THR] Receiver Thread started\n");
  ...
  while (g_running) {
    ...
    if (n <= 0) {
      if (n == 0) {
        fprintf(stderr, "[CELLULAR] Server closed connection cleanly\n");
      } else {
        fprintf(stderr, "[CELLULAR] recv() error: %s (errno=%d)\n", strerror(errno), errno);
      }
      g_running = 0;
      break;
    }
    ...
    if (payload_len > 0) {
      ...
      if (n <= 0) {
        fprintf(stderr, "[CELLULAR] recv() payload error: %s\n", strerror(errno));
        break;
      }
    }
    ...
    case MIH_EXT_LINK_REGISTER_CONFIRM:
      /* 收到 LMI 注册成功确认 */
      printf("[CELLULAR-RX] Received Register Confirm (ID=%u)\n",
             hdr->transaction_id);
      /* 注册成功后检查接口状态 */
      if (check_interface_status(g_ctx.config.interface_name)) {
        printf("[CELLULAR] Interface %s is UP, sending Link_Up_Indication\n",
               g_ctx.config.interface_name);
    ...
    default:
      /* 捕获未知的 MIH 原语 */
      printf("[CELLULAR-RX] Received Unknown Primitive: 0x%04X\n",
             hdr->primitive_type);
      break;
    }
  }
...
static void *dlm_reporting_thread(void *arg) {
  (void)arg;
  printf("[CELLULAR-THR] Reporting Thread started\n");
  ...
  while (g_running) {
    ...
    if (prev_iface_up && !curr_iface_up) {
      printf("[CELLULAR-LINK] Interface DOWN detected: %s\n",
             g_ctx.config.interface_name);
      dlm_physical_link_down(LINK_DOWN_REASON_FAILURE);
    } else if (!prev_iface_up && curr_iface_up) {
      printf("[CELLULAR-LINK] Interface UP detected: %s\n",
             g_ctx.config.interface_name);
      pthread_mutex_lock(&g_ctx.state.mutex);
...
int main(int argc, char **argv) {
  ...
  if (dlm_connect_to_server() != 0) {
    fprintf(stderr, "[CELLULAR] Failed to connect to Standard MIH Server. Is "
                    "app_magic running?\n");
    return -1;
  }

  pthread_t rpt_thread, rx_thread;
