...
typedef enum {
    DLM_LINK_TYPE_SATCOM   = 0x01,
    DLM_LINK_TYPE_CELLULAR = 0x02,
    DLM_LINK_TYPE_WIFI     = 0x03
} dlm_link_type_t;
...
typedef struct {
    uint8_t             bearer_id;
    bool                active;
    uint32_t            allocated_bw_fl;
    uint32_t            allocated_bw_rl;
    uint8_t             cos_id;
    time_t              created_time;
} dlm_bearer_info_t;
...
typedef struct {
    ...
    uint32_t            ip_address;
    uint32_t            netmask;
    int32_t             simulated_rssi;
    uint8_t             signal_quality;
    uint32_t            current_usage_fl;
    uint32_t            current_usage_rl;
    float               utilization_percent;
    dlm_bearer_info_t   bearer_map[MAX_BEARERS];
    uint8_t             num_active_bearers;
    uint8_t             next_bearer_id;
    uint64_t            tx_bytes;
    uint64_t            rx_bytes;
    uint64_t            tx_packets;
    uint64_t            rx_packets;
...
typedef struct {
    ...
    char mihf_path[128];
    char dlm_path[128];
    int  initial_rssi_dbm;
    int  going_down_lead_time_ms;
